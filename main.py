...
from envs import AtariEnv, CPCEnv
...
parser.add_argument('--T-max', type=int, default=int(1060), metavar='STEPS',
...
parser.add_argument('--history-length', type=int, default=4, metavar='T',
                    help='Number of consecutive states processed (ATARI)')  # 1 for MFEC (originally), 4 for MFEC (in NEC paper) and NEC
...
parser.add_argument('--num-neighbours', type=int, default=50, metavar='p',
                    help='Number of nearest neighbours')  # 11 for MFEC, 50 for NEC
parser.add_argument('--model', type=str, metavar='PARAMS', help='Pretrained model (state dict)')
parser.add_argument('--memory-capacity', type=int, default=int(1e5), metavar='CAPACITY',
                    help='Experience replay memory capacity')
# 1e5
parser.add_argument('--dictionary-capacity', type=int, default=int(5e5), metavar='CAPACITY',
                    help='Dictionary capacity (per action)')  # 1e6 for MFEC, 5e5 for NEC
parser.add_argument('--replay-frequency', type=int, default=4, metavar='k', help='Frequency of sampling from memory')
parser.add_argument('--episodic-multi-step', type=int, default=100, metavar='n',
                    help='Number of steps for multi-step return from end of episode')  # Infinity for MFEC, 100 for NEC
parser.add_argument('--epsilon-initial', type=float, default=1, metavar='ε', help='Initial value of ε-greedy policy')
parser.add_argument('--epsilon-final', type=float, default=0.001, metavar='ε',
                    help='Final value of ε-greedy policy')  # 0.005 for MFEC, 0.001 for NEC
parser.add_argument('--epsilon-anneal-start', type=int, default=5000, metavar='ε',
                    help='Number of steps before annealing ε')
parser.add_argument('--epsilon-anneal-end', type=int, default=25000, metavar='ε',
                    help='Number of steps to finish annealing ε')
parser.add_argument('--discount', type=float, default=0.99, metavar='γ',
                    help='Discount factor')  # 1 for MFEC, 0.99 for NEC
...
parser.add_argument('--kernel', type=str, default='mean_IDW', choices=['mean', 'mean_IDW'], metavar='k',
                    help='Kernel function')  # mean for MFEC, mean_IDW for NEC
parser.add_argument('--kernel-delta', type=float, default=1e-3, metavar='δ', help='Mean IDW kernel delta')
parser.add_argument('--batch-size', type=int, default=32, metavar='SIZE', help='Batch size')
parser.add_argument('--learn-start', type=int, default=50000, metavar='STEPS',
                    help='Number of steps before starting training')  # 0 for MFEC, 50000 for NEC
...
# 500
parser.add_argument('--evaluation-epsilon', type=float, default=0, metavar='ε',
                    help='Value of ε-greedy policy for evaluation')
parser.add_argument('--checkpoint-interval', type=int, default=0, metavar='STEPS',
                    help='Number of training steps between saving buffers (0 to disable)')  # TODO
...
metrics = {'train_steps': [], 'train_episodes': [], 'train_rewards': [], 'test_steps': [], 'test_rewards': [],
           'test_Qs': []}
...
if args.algorithm == 'MFEC':
    ...
    agent = NECAgent(args, env.observation_space.shape, env.action_space.n, env.hash_space.shape[0],env.num_columns())
...
while T < args.evaluation_size:
    ...
    T += 1
val_mem.append_batch(np.stack(states), np.zeros((args.evaluation_size,), dtype=np.int64),
...
if args.evaluate:
    ...
    print('Avg. reward: ' + str(sum(test_rewards) / args.evaluation_episodes) + ' | Avg. Q: ' + str(
        sum(test_Qs) / args.evaluation_size))
else:
    # Training loop
    agent.train()
    ...
    for T in tqdm(range(1, args.T_max + 1)):
        ...
        if T > args.epsilon_anneal_start and T <= args.epsilon_anneal_end:
            epsilon -= (args.epsilon_initial - args.epsilon_final) / (
                        args.epsilon_anneal_end - args.epsilon_anneal_start)
            agent.set_epsilon(epsilon)
        ...
        if done:
            ...
            for i in range(episode_T - 1, -1, -1):  # Calculate return-to-go in reverse
                ...
                if episode_T - i > args.episodic_multi_step:  # Calculate multi-step returns (originally only for NEC)
                    multistep_returns[i] = returns[i] + args.discount ** args.episodic_multi_step * (
                                values[i + args.episodic_multi_step] - returns[i + args.episodic_multi_step])
                else:  # Calculate Monte Carlo returns (originally only for MFEC)
                    multistep_returns[i] = returns[i]
            states, actions, returns, keys, hashes = np.stack(states), np.asarray(actions, dtype=np.int64), np.asarray(
                multistep_returns, dtype=np.float32), np.stack(keys), np.stack(hashes)
            unique_actions, unique_action_reverse_idxs = np.unique(actions,
                                                                   return_inverse=True)  # Find every unique action taken and indices
            for i, a in enumerate(unique_actions):
                a_idxs = (unique_action_reverse_idxs == i).nonzero()[0]
                agent.update_memory_batch(a.item(), keys[a_idxs], returns[a_idxs][:, np.newaxis],
                                          hashes[a_idxs])  # Append transition to DND of action in batch
        ...
        if T >= args.learn_start:
            ...
            if T % args.evaluation_interval == 0:
                ...
                log('T = ' + str(T) + ' / ' + str(args.T_max) + ' | Avg. reward: ' + str(
                    sum(test_rewards) / args.evaluation_episodes) + ' | Avg. Q: ' + str(
                    sum(test_Qs) / args.evaluation_size))
                ...
                torch.save(metrics, os.path.join(results_dir, 'metrics.pth'))
df = pd.read_csv('All_estimation_raw_data.csv')[['SubjID', 'GameID', 'Trial', 'B']]
SubjID_list, GameID_list, Trial_list, B_list = [], [], [], []
for key, value in env.decisions.items():
    SubjID_list.append(key[0])
    GameID_list.append(key[1])
    Trial_list.append(key[2])
    B_list.append(value)
decisions = pd.DataFrame.from_dict(
    {'SubjID': SubjID_list, 'GameID': GameID_list, 'Trial': Trial_list, 'B_pred': B_list}).merge(df[df['Trial'] > 5],
                                                                                                 on=['SubjID', 'GameID',
                                                                                                     'Trial'])
decisions.to_csv('decisions.csv', index=False)
print(mean_squared_error(decisions[decisions['Trial']>19]['B'], decisions[decisions['Trial']>19]['B_pred']))
env.close()
