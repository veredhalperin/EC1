...
class NEC(nn.Module):
  def __init__(self,args, observation_shape, action_space, hash_size, num_columns):
    super().__init__()
    faiss_gpu_resources = _setup_faiss_gpu_resources(args.device)
    self.memories = [DND(args, hash_size, faiss_gpu_resources) for _ in range(action_space)]
    self.BatchNormalization1=nn.BatchNorm1d(num_columns)
    self.Dropout1=nn.Dropout(0.1)
    self.Dense1=nn.Linear(num_columns,4096)
    self.BatchNormalization2 = nn.BatchNorm1d(16)
    self.Dropout2=nn.Dropout(0.1)
    self.Conv1d1=nn.Conv1d(in_channels=16,out_channels = 16, kernel_size = 5, bias = False, padding = 0)
    self.swish1 = nn.SiLu
    self.AvgPool1d=nn.AvgPool1d(kernel_size=2)
    self.BatchNormalization3 = nn.BatchNorm1d(16)
    self.Dropout3=nn.Dropout(0.1)
    self.Conv1d2 = nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3, bias=False, padding=0)
    self.swish2=nn.SiLU()
    self.BatchNormalization4 = nn.BatchNorm1d(16)
    self.Dropout4=nn.Dropout(0.1)
    self.Conv1d3 = nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3, bias=False, padding=0)
    self.swish3=nn.SiLU()
    self.MaxPool1D=nn.MaxPool1d(kernel_size=4,stride=2)
    self.BatchNormalization5 = nn.BatchNorm1d(16)
    self.Activation1=nn.SiLU()
    self.Dense2=nn.Linear(16,args.key_size)
    self.Activation2=nn.Sigmoid()
  def forward(self, inp, learning=False):
    x=self.BatchNormalization1(inp)
    x=self.Dropout1(x)
    x=nn.utils.weight_norm(self.Dense(x))
    x=torch.reshape(x,(256,16))
    x=self.BatchNormalization2(x)
    x=self.Dropout2(x)
    x=self.nn.utils.weight_norm(self.swish1(self.Conv1d1(x)))
    x=self.AvgPool1d(x)
    xs=x
    x=self.BatchNormalization3(x)
    x=self.Dropout3(x)
    x = self.nn.utils.weight_norm(self.swish2(self.Conv1d2(x)))
    x=self.BatchNormalization4(x)
    x=self.Dropout4(x)
    x = self.nn.utils.weight_norm(self.swish3(self.Conv1d3(x)))
    x=torch.mul(x,xs)
    x=self.MaxPool1D(x)
    x=torch.flatten(x)
...
class NEC(nn.Module):
  def __init__(self, args, observation_shape, action_space, hash_size,num_columns):
    ...
    self.Linear1 = nn.Linear(num_columns, int(num_columns/2))
    self.Linear2 = nn.Linear(int(num_columns/2), int(num_columns/4))
    self.Linear3 = nn.Linear(int(num_columns/4), int(num_columns/8))
    self.Linear4 = nn.Linear(5684, args.key_size)
    faiss_gpu_resources = _setup_faiss_gpu_resources(args.device)
    self.memories = [DND(args, hash_size, faiss_gpu_resources) for _ in range(action_space)]
  ...
  def forward(self, observation, learning=False):
    ...
    hidden = F.relu(self.Linear1(observation))
    hidden = F.relu(self.Linear2(hidden))
    hidden = F.relu(self.Linear3(hidden))
    keys = F.relu(self.Linear4(hidden.view(-1, 5684)))
    memory_output = [memory(keys, learning) for memory in self.memories]
    if learning:
      memory_output, neighbours, values, idxs = zip(*memory_output)
      return torch.cat(memory_output, dim=1), neighbours, values, idxs, keys  # Return Q-values, neighbours, values and keys
